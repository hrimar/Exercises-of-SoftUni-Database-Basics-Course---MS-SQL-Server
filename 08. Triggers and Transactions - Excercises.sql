...
--2-ро.Създаваме тригера и само него пускаме:
...
--3-то.Тестваме как се пуска тригера и как работи
UPDATE Accounts
SET Balance += 10
WHERE Id = 1
...
AS
BEGIN TRANSACTION
		EXEC usp_WithdrawMoney @SenderId, @Amount     			-– процедурата от зад 2
		EXEC usp_DepositMoney @ReceiverId, @Amount				-– процедурата от зад 3
...
WHERE EmployeeID = 1 --, но триенето е вързано с FK, 
--затова трябва да си добавим на ръка нещо и само него да изтрием
